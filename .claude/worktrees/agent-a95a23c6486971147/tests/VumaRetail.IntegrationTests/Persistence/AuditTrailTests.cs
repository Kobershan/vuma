...
public sealed class AuditTrailTests(PostgresFixture fixture)
{
    ...
    public async Task An_insert_is_stamped_from_the_clock_and_the_principal()
    {
        ...
        await using VumaRetailDbContext context = TestDbContextFactory.For(connectionString, clock, principal);
    ...
    public async Task An_update_leaves_the_creation_stamp_alone()
    {
        ...
        await using VumaRetailDbContext context = TestDbContextFactory.For(connectionString, clock);
    ...
    public async Task A_detached_entity_cannot_rewrite_its_own_origin()
    {
        ...
        await using (VumaRetailDbContext writer = TestDbContextFactory.For(connectionString, principal: new TestPrincipalAccessor("user:alice")))
        {
        ...
        await using (VumaRetailDbContext attacker = TestDbContextFactory.For(connectionString, principal: new TestPrincipalAccessor("user:mallory")))
        {
        ...
        await using VumaRetailDbContext reader = TestDbContextFactory.For(connectionString);
