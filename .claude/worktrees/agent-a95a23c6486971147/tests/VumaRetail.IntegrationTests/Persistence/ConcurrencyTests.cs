...
public sealed class ConcurrencyTests(PostgresFixture fixture)
{
    ...
    public async Task The_second_writer_to_a_changed_row_is_refused()
    {
        ...
        await using VumaRetailDbContext first = TestDbContextFactory.For(connectionString);
        await using VumaRetailDbContext second = TestDbContextFactory.For(connectionString);
