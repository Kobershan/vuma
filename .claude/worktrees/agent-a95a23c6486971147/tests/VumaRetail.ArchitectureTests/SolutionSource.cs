...
public static class SolutionSource
{
    ...
    /// <summary>The repository root — the directory holding <c>VumaRetail.sln</c>.</summary>
    ...
    private static DirectoryInfo FindRoot()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "VumaRetail.sln")))
        {
            directory = directory.Parent;
        }

        return directory ?? throw new InvalidOperationException(
            "Could not find VumaRetail.sln above the test output directory.");
    }
}
