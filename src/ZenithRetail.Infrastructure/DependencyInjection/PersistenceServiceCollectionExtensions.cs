...
using ZenithRetail.Infrastructure.Persistence.Interceptors;
using ZenithRetail.Infrastructure.Security;
using ZenithRetail.Infrastructure.Time;
...
{
    /// <summary>
    /// Registers <see cref="ZenithRetailDbContext"/>, the audit interceptor and the ambient
    ...
    /// </remarks>
    public static IServiceCollection AddZenithPersistence(this IServiceCollection services, string connectionString)
    {
        ...
        services.AddDbContext<ZenithRetailDbContext>((provider, options) =>
        {
        ...
        services.AddScoped<IUnitOfWork>(provider => provider.GetRequiredService<ZenithRetailDbContext>());
