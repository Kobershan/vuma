...
public sealed class JwtOptions
{
    ...
    public const string SectionName = "Zenith:Jwt";
    ...
    public string Issuer { get; set; } = "zenith-store-server";

    /// <summary>Who the token is for.</summary>
    public string Audience { get; set; } = "zenith";
...
/// a third party has to validate a Zenith token, which is Stage 20's problem and not this one's.
/// </para>
/// </remarks>
...
public sealed class JwtTokenIssuer(JwtOptions options, IClock clock) : ITokenIssuer
{
    ...
    public AccessToken Issue(TokenSubject subject)
    {
        ...
        [
            ...
            new(ZenithClaims.TenantId, subject.TenantId.ToString()),
            new(ZenithClaims.SecurityStamp, subject.SecurityStamp),
        ];

        if (subject.StoreId is { } storeId)
        {
            claims.Add(new Claim(ZenithClaims.StoreId, storeId.ToString()));
        }

        if (subject.TerminalId is { } terminalId)
        {
            claims.Add(new Claim(ZenithClaims.TerminalId, terminalId.ToString()));
        }
