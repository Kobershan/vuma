...
{
    private DeliveryRun(Guid tenantId, Guid? storeId, Guid? carrierId, Guid? vehicleId, string runNumber, DateOnly plannedDate, string? driverName, string? vehicleRegistration) : base(tenantId, storeId)
    { CarrierId = carrierId; VehicleId = vehicleId; RunNumber = runNumber; PlannedDate = plannedDate; DriverName = driverName; VehicleRegistration = vehicleRegistration; Status = LogisticsRunStatus.Planned; }
    private DeliveryRun() { }
    ...
    public static DeliveryRun Create(Guid tenantId, Guid? storeId, Guid? carrierId, string runNumber, DateOnly plannedDate, string? driverName, string? vehicleRegistration, Guid? vehicleId = null)
    {
        ...
        return new DeliveryRun(tenantId, storeId, carrierId, vehicleId, runNumber.Trim(), plannedDate, Clean(driverName, 160), Clean(vehicleRegistration, 32));
    }
