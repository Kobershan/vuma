...
public static class LogisticsEndpoints
{
    public static IEndpointRouteBuilder MapVumaLogistics(this IEndpointRouteBuilder endpoints)
    {
        ...
        api.MapPost("/shipments/{shipmentId:guid}/exception", async (Guid shipmentId, IDispatcher dispatcher, CancellationToken ct) => { await dispatcher.SendAsync(new MarkShipmentExceptionCommand(shipmentId), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.ShipConfirm);
        api.MapPost("/shipments/{shipmentId:guid}/cancel", async (Guid shipmentId, IDispatcher dispatcher, CancellationToken ct) => { await dispatcher.SendAsync(new CancelShipmentCommand(shipmentId), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.ShipConfirm);
        ...
        api.MapPost("/runs/{runId:guid}/dispatch", async (Guid runId, IDispatcher dispatcher, CancellationToken ct) => { await dispatcher.SendAsync(new DispatchDeliveryRunCommand(runId), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.PickManage);
        api.MapPost("/runs/{runId:guid}/complete", async (Guid runId, IDispatcher dispatcher, CancellationToken ct) => { await dispatcher.SendAsync(new CompleteDeliveryRunCommand(runId), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.PickManage);
        api.MapPost("/runs/{runId:guid}/cancel", async (Guid runId, IDispatcher dispatcher, CancellationToken ct) => { await dispatcher.SendAsync(new CancelDeliveryRunCommand(runId), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.PickManage);
        api.MapPost("/stops/{stopId:guid}/start", async (Guid stopId, IDispatcher dispatcher, CancellationToken ct) => { await dispatcher.SendAsync(new StartDeliveryStopCommand(stopId), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.PickManage);
        api.MapPost("/stops/{stopId:guid}/complete", async (Guid stopId, bool successful, IDispatcher dispatcher, CancellationToken ct) => { await dispatcher.SendAsync(new CompleteDeliveryStopCommand(stopId, successful), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.PickManage);
        api.MapGet("/vehicles", async (string? status, ILogisticsRepository repo, CancellationToken ct) => Results.Ok((await repo.ListVehiclesAsync(Enum.TryParse<VehicleStatus>(status, true, out var parsed) ? parsed : null, ct)).Select(x => new VehicleResponse(x.Id, x.StoreId, x.Registration, x.Make, x.Model, x.Type.ToString(), x.Vin, x.CapacityKg, x.OdometerKm, x.NextServiceOn, x.Status.ToString())))).RequirePermission(WarehousePermissions.View);
        api.MapPost("/vehicles", async (CreateVehicleRequest request, IDispatcher dispatcher, CancellationToken ct) => { if (!Enum.TryParse<VehicleType>(request.Type, true, out var type)) { return Results.BadRequest(new { error = "Invalid vehicle type." }); } var id = await dispatcher.SendAsync(new CreateVehicleCommand(request.StoreId, request.Registration, request.Make, request.Model, type, request.Vin, request.CapacityKg, request.OdometerKm, request.NextServiceOn), ct); return Results.Created($"/api/v1/logistics/vehicles/{id}", new { id, request.Registration, status = VehicleStatus.Active.ToString() }); }).RequirePermission(WarehousePermissions.PickManage);
        api.MapPost("/vehicles/{vehicleId:guid}/odometer", async (Guid vehicleId, decimal odometerKm, IDispatcher dispatcher, CancellationToken ct) => { await dispatcher.SendAsync(new RecordVehicleOdometerCommand(vehicleId, odometerKm), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.PickManage);
        api.MapPost("/vehicles/{vehicleId:guid}/status", async (Guid vehicleId, string status, IDispatcher dispatcher, CancellationToken ct) => { if (!Enum.TryParse<VehicleStatus>(status, true, out var parsed)) { return Results.BadRequest(new { error = "Invalid vehicle status." }); } await dispatcher.SendAsync(new SetVehicleStatusCommand(vehicleId, parsed), ct); return Results.NoContent(); }).RequirePermission(WarehousePermissions.PickManage);
        return endpoints;
    }
