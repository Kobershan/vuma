...
public static class RegistryEndpoints
{
    ...
    public static IEndpointRouteBuilder MapVumaRegistry(this IEndpointRouteBuilder endpoints)
    {
        ...
        {
            if (!Enum.TryParse<BusinessType>(request.BusinessType, true, out var type) || !Enum.TryParse<TransferCostingMethod>(request.TransferCostingMethod, true, out var costing) || !Enum.TryParse<DiscrepancyOwner>(request.DiscrepancyDefaultOwner, true, out var owner))
            {
                return Results.BadRequest("Unknown business type, costing method or discrepancy owner.");
            }
