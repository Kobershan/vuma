...
public static class RegistryEndpoints
{
    ...
    public static IEndpointRouteBuilder MapVumaRegistry(this IEndpointRouteBuilder endpoints)
    {
        ...
        stage22.MapPost("/transfers", async (CreateTransferRequest request, IStage22RegistryService service, CancellationToken ct) => { var transfer = await service.CreateTransferAsync(request.RequesterCompanyId, request.SenderCompanyId, request.ReceiverCompanyId, request.HoldingCompanyId, request.TotalValue, request.CentralBuying, ct); return Results.Created($"/api/v1/stage22/transfers/{transfer.Id}", new Stage22TransferResponse(transfer.Id, transfer.RequesterCompanyId, transfer.SenderCompanyId, transfer.ReceiverCompanyId, transfer.TotalValue, transfer.Status.ToString(), transfer.ReceivedQuantity, transfer.DiscrepancyQuantity)); }).RequirePermission(PlatformPermissions.CompanyManage);
