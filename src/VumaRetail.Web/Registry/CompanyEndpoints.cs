...
public static class CompanyEndpoints
{
    ...
    public static IEndpointRouteBuilder MapVumaCompanies(this IEndpointRouteBuilder endpoints)
    {
        ...
        {
            if (!http.Request.Headers.TryGetValue(SelectionHeader, out var value) || !Guid.TryParse(value, out var companyId))
            {
                throw new InvalidOperationException("COMPANY_SELECTION_REQUIRED");
            }
            ...
            if (!db.Companies.Any(x => x.Id == companyId && x.TenantId == tenant.TenantId && x.IsActive))
            {
