...
public sealed class CreateJourneyDefinitionCommandHandler(IJourneyDefinitionRepository journeys, ITenantContext tenant, ICompanyContext company) : ICommandHandler<CreateJourneyDefinitionCommand, Guid>
{
    ...
    { ArgumentNullException.ThrowIfNull(c); if (company.CompanyId != c.CompanyId) throw new InvalidOperationException("The marketing company is not active."); var j = JourneyDefinition.Create(tenant.TenantId, c.CompanyId, c.Name, c.Version, c.DefinitionJson); journeys.Add(j); return Task.FromResult(j.Id); }
}
public sealed class PublishJourneyDefinitionCommandHandler(IJourneyDefinitionRepository journeys, ICompanyContext company) : ICommandHandler<PublishJourneyDefinitionCommand, Unit>
{
    ...
    { ArgumentNullException.ThrowIfNull(c); var j = await journeys.FindAsync(c.JourneyId, ct) ?? throw new KeyNotFoundException("Journey was not found."); if (j.CompanyId != company.CompanyId) throw new InvalidOperationException("The marketing company is not active."); j.Publish(); return Unit.Value; }
}
...
public sealed class RecordAttributionEventCommandHandler(IAttributionEventRepository events, ITenantContext tenant, ICompanyContext company, IClock clock) : ICommandHandler<RecordAttributionEventCommand, Guid>
{
    ...
    { ArgumentNullException.ThrowIfNull(c); if (company.CompanyId != c.CompanyId) throw new InvalidOperationException("The marketing company is not active."); var e = AttributionEvent.Record(tenant.TenantId, c.CompanyId, c.CampaignId, c.MessageId, c.CustomerId, c.EventType, clock.UtcNow); events.Add(e); return Task.FromResult(e.Id); }
}
