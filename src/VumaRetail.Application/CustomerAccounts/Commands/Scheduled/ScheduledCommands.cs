...
namespace VumaRetail.Application.CustomerAccounts.Commands.Scheduled;

public sealed record ExpiryOutcome(int Expired, int Reminded);

[CommandSideEffect(SideEffect.Write)]
public sealed record ExpireLayByAgreementsCommand : ICommand<ExpiryOutcome>;

public sealed class ExpireLayByAgreementsCommandHandler(
    ILayByAgreementRepository laybys,
    IStockReservationRepository holds,
    IReservationService reservations,
    INotificationDispatcher notifications,
    IClock clock)
    : ICommandHandler<ExpireLayByAgreementsCommand, ExpiryOutcome>
{
    public async Task<ExpiryOutcome> HandleAsync(ExpireLayByAgreementsCommand command, CancellationToken cancellationToken = default)
    {
        ...
        return new ExpiryOutcome(expired, reminded);
    }
}

[CommandSideEffect(SideEffect.Write)]
public sealed record AccrueAccountInterestCommand : ICommand<int>;

public sealed class AccrueAccountInterestCommandHandler(
    ...
    IClock clock)
    : ICommandHandler<AccrueAccountInterestCommand, int>
{
    public async Task<int> HandleAsync(AccrueAccountInterestCommand command, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(command);
