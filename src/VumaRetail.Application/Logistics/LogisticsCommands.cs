...
[CommandSideEffect(SideEffect.Write)]
public sealed record CreateDeliveryRunCommand(Guid? StoreId, Guid? CarrierId, string RunNumber, DateOnly PlannedDate, string? DriverName, string? VehicleRegistration, Guid? VehicleId = null) : ICommand<Guid>;
[CommandSideEffect(SideEffect.Write)]
    ...
    ICommandHandler<CreateCarrierCommand, Guid>, ICommandHandler<CreateShipmentCommand, Guid>, ICommandHandler<DispatchShipmentCommand, Unit>, ICommandHandler<MarkShipmentExceptionCommand, Unit>, ICommandHandler<CancelShipmentCommand, Unit>, ICommandHandler<RecordPodCommand, Guid>, ICommandHandler<CreateDeliveryRunCommand, Guid>, ICommandHandler<AddDeliveryStopCommand, Guid>, ICommandHandler<DispatchDeliveryRunCommand, Unit>, ICommandHandler<CompleteDeliveryRunCommand, Unit>, ICommandHandler<CancelDeliveryRunCommand, Unit>, ICommandHandler<StartDeliveryStopCommand, Unit>, ICommandHandler<CompleteDeliveryStopCommand, Unit>, ICommandHandler<CreateVehicleCommand, Guid>, ICommandHandler<RecordVehicleOdometerCommand, Unit>, ICommandHandler<SetVehicleStatusCommand, Unit>
{
    ...
    public async Task<Guid> HandleAsync(CreateDeliveryRunCommand c, CancellationToken ct = default) { ArgumentNullException.ThrowIfNull(c); if (c.CarrierId is Guid carrierId && (await repo.FindCarrierAsync(carrierId, ct).ConfigureAwait(false) is not { IsActive: true })) throw new KeyNotFoundException("Active carrier not found."); if (c.VehicleId is Guid vehicleId && (await repo.FindVehicleAsync(vehicleId, ct).ConfigureAwait(false) is not { Status: VehicleStatus.Active })) throw new InvalidOperationException("Only an active vehicle can be assigned to a delivery run."); var x = DeliveryRun.Create(tenant.TenantId, c.StoreId, c.CarrierId, c.RunNumber, c.PlannedDate, c.DriverName, c.VehicleRegistration, c.VehicleId); repo.Add(x); return x.Id; }
