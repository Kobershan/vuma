...
public sealed record CreateDeliveryRunRequest(Guid? StoreId, Guid? CarrierId, string RunNumber, DateOnly PlannedDate, string? DriverName, string? VehicleRegistration, Guid? VehicleId = null);
...
public sealed record DeliveryRunResponse(Guid Id, string RunNumber, DateOnly PlannedDate, Guid? CarrierId, Guid? VehicleId, string? DriverName, string? VehicleRegistration, string Status, IReadOnlyList<DeliveryStopResponse> Stops);
