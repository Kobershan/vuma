...
/// <summary>
/// One tenant, one store, and the <c>catalog</c> module wired over a real database — through the
/// Stage 03 dispatcher and its pipeline. Same shape as <c>IdentityHarness</c>.
/// </summary>
public sealed class CatalogHarness : IAsyncDisposable
{
    private readonly ServiceProvider _services;

    private CatalogHarness(VumaRetailDbContext context, TestTenantContext tenant, TestClock clock, Guid tenantId, Guid storeId)
    {
        Context = context;
        ...
        Units = new UnitOfMeasureRepository(context);
        Items = new ItemRepository(context);
        Variants = new ItemVariantRepository(context);
        Barcodes = new BarcodeRepository(context);
        ...
        services.AddSingleton(Variants);
        services.AddSingleton(Barcodes);
    ...
    /// <summary>Unit of measure repository.</summary>
    public IUnitOfMeasureRepository Units { get; }

    /// <summary>Item repository.</summary>
    public IItemRepository Items { get; }

    /// <summary>Item variant repository.</summary>
    public IItemVariantRepository Variants { get; }

    /// <summary>Barcode repository.</summary>
    ...
    public static async Task<CatalogHarness> CreateAsync(PostgresFixture fixture)
    {
        ...
        Tenant seeded = Tenant.CreateWithSouthAfricanDefaults("Catalog Harness (Pty) Ltd", "Harness");
        seeded.Activate();
        ...
        return new CatalogHarness(context, tenant, clock, seeded.Id, store.Id);
    }
