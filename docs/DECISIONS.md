...
## ADR-149 � Stage 15 planning rules: forecast set, safety-stock math, OTB as warning, suggestions that commit nothing, markdowns through Stage 10 � **PROPOSED**
**Context.** Stage 15 (merchandise planning, forecasting, replenishment) needed five irreversible choices: which forecast methods, which safety-stock formula, whether open-to-buy blocks, what a suggestion may do on its own, and who owns markdown pricing.
**Decision.** (1) Three forecast methods � moving-average, exponential-smoothing, seasonal-naive � behind one pure `IForecastEngine` seam, weekly Monday�Sunday buckets, MAPE skipping zero-actual periods, thin input returning a flagged low-confidence result rather than throwing. (2) Safety stock is variance-based (z � sweekly � v(lead-time days / 7)) with an 8-week minimum; below it the fallback (lead-time days � mean daily demand � 25% buffer) is flagged `LowConfidence`, and a horizon that cannot cover lead time plus review period refuses loudly. (3) Open-to-buy is `Remaining = Planned - Committed` with commitments read live (submitted/approved requisitions, open orders; cancelled documents never count) � a warning flag on suggestions, never a blocker. (4) A suggestion only creates a downstream document on explicit accept (exactly-once via status transition); transfer candidates are generated only with an active `SharedSourcing` link checked at generation. (5) Markdowns go through Stage 05 approval and activate as Stage 10 percentage-off promotions � planning never writes `sales.promotions` directly, and unapproved plans price nothing. ABC ranks by demand-quantity share (a documented proxy; cost joins stay in Stage 08 valuation).
**Consequences.** Thresholds (20% sell-through, 90 days of supply, C/Z classes) and the 25% fallback buffer are constants on the planner/calculator, changeable in code review rather than configuration.


## ADR-150 — Stage numbering correction (19 = CRM, 20 = loyalty), `path/to/` scaffold removal, `vendor/` convention — **PROPOSED**

**Context.** A task brief asked for `docs/stages/STAGE-10-crm.md`, but `docs/ROADMAP.md` (authoritative on stage numbers) already assigns Stage 10 to Sales Management & Promotions (DONE, `docs/stages/STAGE-10-sales-promotions.md`) and assigns CRM to Stage 19 and Loyalty to Stage 20. The same brief asked for a `path/to/Vuma.Retail` folder fix, which resolved to `path/to/VumaRetail.Infrastructure/Persistence/` — two stale `DesignTimeDbContextFactory` duplicates committed by an unattended scaffolding run — plus a home for a vendored Proxima Orbit loyalty engine.
**Decision.** (1) The CRM stage document is written as `docs/stages/STAGE-19-crm.md`, not `STAGE-10-crm.md`; no second Stage-10 document exists. (2) The `path/to/` tree is deleted outright: the live files already exist at `src/VumaRetail.Infrastructure/Persistence/`, nothing referenced the copies (no `.sln`/`.csproj`/CI/docs links), and they were an older SQL-Server/LocalDB variant superseded by the Npgsql versions. (3) Third-party vendored code lives under `vendor/<Product>/` (established by `docs/stages/STAGE-20-loyalty-public-api.md` §Vendoring approach: `vendor/ProximaOrbit/`, subtree merge, compiled into the solution, called over HTTP at runtime); `src/` stays first-party `VumaRetail.*` only. (4) Stage 19/20 domain scaffolding (`Domain/Crm`, `Domain/Loyalty`) ships as namespaced folders inside `VumaRetail.Domain` — no new assemblies — because the SDK auto-includes them and separate module assemblies would need DI/architecture-test registration the full stages have not defined yet.
**Consequences.** `docs/tasks/MISSING-SPECS.md` still lists 19/20/21 as needing specs; 19 and 20 now have stage documents but still need task decomposition (`19-MAP-01`/`20-MAP-01` are planning gates). The 44 scaffolding tests pin the documented contracts; the full stage builds may replace the scaffolding but must keep the tests green or supersede them explicitly.

## ADR-151 — Clearing intents link to their allocation by id, not by company-plus-amount — **PROPOSED**
**Context.** `FindClearingIntentAsync` matched a clearing to its allocation on `(ToCompanyId, Amount)`. One receipt may carry several same-company, same-amount slices, so the match is ambiguous: the randomised 200-op proof left a `Pending` clearing dangling because the lookup returned its settled twin and the pending one was never compensated on reversal.
**Decision.** `InterCompanyClearingIntent` carries a required `AllocationId` (migration `Stage07c_ClearingAllocationLink`, reversible, `Down` executed on scratch PG); the lookup matches the exact allocation first and falls back to the legacy company-plus-amount match only for intents created before the link existed (`AllocationId == Guid.Empty`).
**Consequences.** Same-company, same-amount slices on one receipt settle and reverse independently. The fallback keeps old rows readable; no backfill is needed because every clearing created since 07C-004 carries the link.

## ADR-152 — Outbound group payments deferred: 07c ships inbound receipting only — **PROPOSED**
**Context.** TASK-07C-004's scope asked for an `IGroupPaymentService` implementation plus `/api/v1/group-payments` endpoints, or a written deferral. The inbound path (capture once, allocate across companies, clear, reverse, close-guard) is now proven against real databases; the outbound path is a second full saga (supplier-side apportionment, `ApPayment` legs, its own clearing direction and reversal) with no task decomposition behind it.
**Decision.** 07c ships inbound only. The `GroupPaymentRun`/`GroupPaymentAllocation` domain types and `IGroupPaymentService` port stay as the contract the outbound stage will implement; no endpoints are exposed until the legs behind them exist (R3 — no feature without its API, and no API without its feature). The stage document's endpoint table stands as the outbound stage's starting spec.
**Consequences.** `POST /api/v1/group-payments` returns 404 until the outbound stage lands. Nothing in the inbound path assumes outbound exists; the clearing table and link conventions are shared by design so the outbound saga reuses them.

## ADR-153 — Stage 19/20 completion: scaffolding supersession, public surface served by the store server, Orbit faked — **PROPOSED**
**Context.** Commit `b6b83be` left Stage 19/20 as stage docs plus Domain scaffolding that violates three enforced rules: wall-clock reads (`PersistenceRulesTests.Nothing_reads_the_wall_clock_except_SystemClock`), `new`-shadowed audit columns, and a `Segment` outside the `Entity` base (no `company_id`, no replication declaration). The Stage-20 doc assumes a `vendor/ProximaOrbit` subtree with no upstream to merge, and a standalone public host whose persistence wiring the `PublicApi_cannot_reach_internal_contracts_or_the_domain_model` test forbids (PublicApi may not touch Infrastructure, Domain, Contracts or Web).
**Decision.** (1) The scaffolding is replaced, not extended (explicit supersession per ADR-150): every domain method takes its timestamp, every entity carries `company_id`, `Consent` moves to `Domain.Crm`, and the 44 tests keep their intent with updated call sites. (2) The public loyalty surface (DTOs, mappings, rate limiting, neutral read-only) lives in `VumaRetail.PublicApi` with Application-only dependencies and is served by the store server via `MapVumaLoyaltyPublic`; the standalone API-key/member-token deployable follows with Stage 30b's auth, and `Program.cs` is removed until then. (3) Orbit arrives as the `IOrbitClient` port with the thread-safe `InMemoryOrbitClient` default and a config-gated `HttpOrbitClient`; `vendor/` stays empty until a real upstream exists. (4) `LoyaltyTransaction` is a mutable state machine with full audit history (the `ReplenishmentSuggestion` precedent), not an `IImmutableRecord` — the audit stamper proved the marker wrong on first contact with a real database. (5) ConvertLead is link-only in v1 (no create-customer port exists in Partners).
**Consequences.** No new assemblies; metering arrives by schema declaration; the read-only sweep and command-classification tests inherit the 24 new commands with no edits. Deferred with written reasons: standalone public host + API-key auth (30b), real Orbit credentials, lead auto-create, dynamic-segment query language, consent/activity retention sweeper, GL posting of redemptions.

## ADR-154 — Audit stage specifications and integration planning — PROPOSED

**Context.** The 2026-09-12 owner-requested audit found ten missing subject specifications, a Stage 22 naming collision, and gaps between existing clients/host composition and the required offline-first local/cloud/mobile product. The owner also requested Proxima Orbit integration and protection against unauthorized copying/modification.
