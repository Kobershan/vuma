...
TEST STATUS: `dotnet test tests/VumaRetail.UnitTests/VumaRetail.UnitTests.csproj --no-restore` — 828 passed; registry filter — 12 passed; architecture tests — 35 passed; `dotnet build src/VumaRetail.Infrastructure/VumaRetail.Infrastructure.csproj --no-restore` — 0 warnings, 0 errors; registry persistence tests — 2 failed before execution because Docker/local PostgreSQL is unavailable.
