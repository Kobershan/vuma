...
Verification recorded 2026-09-13: `HrStagesRulesTests` (6/6), `HrLifecycleTests` (5/5), and `EmployeeDocumentTests` (4/4) pass. Employee documents store only validated external blob metadata and a checksum. Suspend, activate and terminate employee lifecycle commands/routes are now implemented. The disciplinary-case domain foundation is covered by `DisciplinaryCaseTests` (2/2); persistence, workflow routes and payroll-export contracts remain open.
