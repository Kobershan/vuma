...
**Stage 19/20 docs + `path/to/` fix + scaffolding tests (2026-09-10, operator-tasked):** (a) Wrote `docs/stages/STAGE-19-crm.md` (NOT_STARTED, Depends on 10) - CRM owns leads/opportunities/activities/segments/consent layered on Stage 06's `catalog.Customer`, which it never creates; consent state machine + POPIA rules; internal ports incl. the `IConsentService.IsConsentValidAsync` contract Stage 20 consumes; hooks left for 10/20/22/29. Numbering correction: the brief asked for `STAGE-10-crm.md`, but ROADMAP assigns 10 to Sales & Promotions (DONE) and CRM to 19 - recorded as ADR-150, no duplicate Stage-10 doc created. (b) Wrote `docs/stages/STAGE-20-loyalty-public-api.md` (NOT_STARTED, Depends on 10, 19) - Vuma owns the front-facing experience + public API, vendored Proxima Orbit owns earn/burn/tiers/ledger; `vendor/ProximaOrbit/` via subtree merge, compiled in, called over HTTP at runtime (ADR-150); earn/burn flow with queue-and-retry degraded mode (till never blocks); full public endpoint list with Orbit mapping, idempotency via `LoyaltyTransaction` + `IdempotencyKey`, rate limits, re-emitted webhooks. Stage 19 has no implementation yet (only the doc written here) - flagged as an assumption. `docs/API_LOYALTY.md` (cited by CLAUDE.md s6) still does not exist; the Stage-20 doc is the interim contract. (c) Deleted `path/to/VumaRetail.Infrastructure/Persistence/` (2 stale SQL-Server/LocalDB `DesignTimeDbContextFactory` duplicates from scaffolding commit `4c8d0a4`; live Npgsql versions already at `src/VumaRetail.Infrastructure/Persistence/`; zero references in `.sln`/`.csproj`/CI/docs). (d) Scaffolding domain (`Domain/Crm`: Lead/Activity/Segment; `Domain/Loyalty`: Consent/LoyaltyMember/LoyaltyTransaction) + 44 unit tests (`UnitTests/Crm`: 17; `UnitTests/Loyalty`: 27 - points calc incl. 49.9950-store/50.00-display rounding, tiers, expiry, mocked-Orbit earn/redeem/idempotency/concurrency/failure-mode). Evidence: `UnitTests` 1249/1249 green (1205 pre-existing + 44 new), `Domain` build 0 errors, no `.csproj` changes (SDK auto-include). 7 first-run failures, all scaffolding defects (missing `WithdrawnAt` stamp, wrong rounding expectation, no idempotency store, invocation-vs-success counting), each fixed with a worked cause. Deferred: Integration/DB tests, PublicApi endpoints, migration, seed, permissions, sync registry, agent panel - all belong to the real Stage 19/20 builds. Pre-existing dirty tree untouched: Stage 07c on-account work + `GroupReceiptHarness.cs` integration errors predate this session and were not modified. ADR-150.
...
**Stage 08c — COMPLETE, merged to `main` (2026-09-07):** TASK-08C-002 sourcing and split fulfilment implemented (Domain `SourcingPlan`/`SourcingAllocation`/`SourcingDemandLine`, Application `AvailabilityThenProximity` strategy, `PlanSourcingQuery`, `CommitSourcingPlanCommand`→`SourcingCommitService`, `ISplitDocumentBuilder`, `IReservationExpiryPolicy`+`ReservationExpiryService`+`ReservationExpiryHostedService`, `SourcingPlannerService`, `SalesOrder.GroupDocumentRef` nullable+index, all three API endpoints). TASK-08C-003 verification executed on real PostgreSQL: all 8 acceptance criteria PASS (12+8 allocation, 15+5 backorder, concurrent race, stale projection → 3 held, two-company commit failure compensates, expiry frees available, rebuild equality, split reconciliation exact sums). 1010 unit + 54 architecture + 23 inventory integration tests green on real PostgreSQL. Migration `Down` tested, both contexts clean. `TradingGroupGuardTests` row added. `docs/DATA_MODEL.md` §4m extended, replication registry updated. Committed and pushed to `main`.
...
| 05 | Workflow, approvals, notifications, documents | **DONE** (main) | Merged via PR #1/#2 | 2026-08-12 |
| 06 | Master data — items, variants, barcodes, UoM, partners | **DONE** (main) | 2026-08-14 |
| 06c | Multi-company foundation — **one database per company** plus the tenant registry, connection routing, provisioning, migration fan-out, per-database backup and sync | **DONE** (main) | Merged 2026-08-27 | 2026-08-27 |
| 06d | Group services — saga coordinator, credit groups with hold tokens, barcode routing index, group read models | **DONE** (main) | Merged | — |
| 06e | Trading group — Operator ID, company links and scopes, shared premises, cross-company users and tills, billing dimensions | **DONE** (main) | Merged | 2026-09-04 |
...
| 07c | Cross-company money — group receipting and allocation, inter-company clearing, consolidated reporting | **DONE** (main) | All layers implemented; code on main | 2026-09-04 |
...
| 29 | Reporting & admin dashboard API | **COMPLETE for repository-deliverable code** — persistence, exports, schedules, dashboard measures, hosted execution and scope/replay tests verified; provider/object-storage production integrations are deployment boundaries | 2026-09-17 |
| 30 | Android admin app | **COMPLETE for repository-deliverable code** — Compose, Room, encrypted session storage, authenticated API client, queue/session safety and CI build verified; physical-device/live accessibility acceptance is deployment evidence | 2026-09-17 |
| 30b | Vendor control plane, metering & SaaS billing | **COMPLETE for repository-deliverable code** — device API, metering/billing/dunning, abuse/fleet/support/provisioning policy and 23 control-plane tests verified; production gateway/KMS/vendor deployment is external | 2026-09-17 |
| 31 | Hardening, DR, packaging & release | **COMPLETE for repository-deliverable code** — release tooling, signatures, activation/rollback, DR drill and CI gates verified; live operational rehearsal and production credentials are external | 2026-09-17 |
...
### 4.26 Imports: `RollbackImportBatchCommand` lacks the idempotency branch its sibling `Commit` has — **RESOLVED and VERIFIED 2026-09-19**
...
### 4.25 `architecture-guard`: CLAUDE.md §7 rule 13 claims an architecture test enforces it — **RESOLVED and VERIFIED 2026-09-19**
...
### 4.24 Cross-cutting Definition-of-Done gaps across Stages 10–13, found by `stage-verifier` executing (not asserting) CLAUDE.md §8 — **PARTIALLY RESOLVED 2026-09-19**
...
2. ~~**Stage 11's own coverage is below the mandatory 80% floor**, measured directly: 1470/1914 = **76.80%** on Domain+Application. Weak spots: `ImportValueParser` 40.4%, `ImportQueries` 38.6%, `ImportExceptions` 49.2%. Stage 11's own exit checklist notably never quoted a coverage number at all, unlike Stages 10/12/13 — which is consistent with this being a real, previously-unmeasured shortfall rather than a regression introduced later.~~ **RESOLVED 2026-09-15 (TASK-004):** meaningful import aggregate coverage tests were added; combined full-unit plus Imports-integration `dotnet-coverage` measures **3187/3926 = 81.18%** on Stage 11 Domain/Application import code.
...
The 2026-09-19 verification supersedes the three stale sub-findings: schema-derived OpenAPI examples are now generated for every JSON request body and enforced by a live contract test; Stage 11 import coverage remains resolved at 81.18%; and checked-in API contract tests cover the high-risk procurement/warehouse permission refusals.

### 4.23 Module entitlement gate confirmed to have zero call sites across Sales, Imports, Procurement and Warehouse too, not only POS — **RESOLVED and VERIFIED 2026-09-19**
...
### 4.22 No replicated entity from Sales, Imports, Procurement or Warehouse — and no `Terminal` node — has ever been exercised by a replication test — **PARTIALLY RESOLVED 2026-09-19**
...
### 4.21 Sales: a return can be double-refunded, one path by a genuine concurrency race and one by a plain retry — **RESOLVED and VERIFIED 2026-09-19**
...
### 4.20 Procurement: a supplier invoice can be paid twice through the three-way match's own designed exception path, a delivery check-in has no retry protection, and the match's audit fields are silently mislabeled — **RESOLVED and VERIFIED 2026-09-19**
...
### 4.19 Warehouse: pick allocation is advisory only, retried task/putaway/move commands double their stock effect, and a cycle count can double-count stock that was picked but not yet confirmed — **RESOLVED and VERIFIED 2026-09-19**
...
The implementation and regression suites now supersede the historical defect descriptions above:

- sales returns lock the parent sale and sibling return rows in the transaction and replay an existing return id;
- procurement invoice release locks the purchase order and its release regression test passes;
- warehouse pick release locks candidate bins and persists reservations;
- rollback replay returns the stored batch counters;
- approval and module entitlement architecture/API guards are present and passing;
- a terminal-tier replication test now proves two-database apply plus replay deduplication.

The remaining coverage gap in §4.22 is the broader entity matrix, not an observed production failure.
...
Integration 507/507 (via `scripts/test.sh`, local PG18 — Docker unavailable here).
...
- Warehouse HTTP surface aligned to contracts/commands (bin/move/putaway/pick/cycle-count
  ...
  building and 500'd every HTTP request — now scalar query params.
...
- Permission catalogue crashed startup twice: `FieldSalesPermissions.Module` was `"field-sales"`
  ...
  manifest + `RequireModule` arg; approval-policy key `"field-sales"` untouched — separate
...
### TASK-07C-004 leg-dispatch rework — COMPLETE (2026-09-10, real PostgreSQL :55432 via `scripts/pg-test.sh`)

Structural gaps from TASK-07C-003 closed: `GroupReceiptLegDispatcher` (new) executes allocation/clearing/reversal legs in their companies' databases, one serialisable transaction per leg, deterministic ids, idempotent replay; dead `GroupReceiptLegHandler`/`GroupReceiptReversalLegHandler` deleted. Clearing intents created on the allocate path and settled after both legs ack. `IPeriodCloseGuard`/`PeriodCloseGuard` wired into `ClosePeriodCommandHandler` as optional deps. `RetryAllocation` command + handler; reverse + retry endpoints. `ar_invoice_id` nullable (on-account slices) via `20260909190342_Stage07c_OnAccountAllocations`.

Evidence: `dotnet build -c Release` 0 errors, Domain/Application 0 warnings; unit 1249/1249; `GroupReceiptLegsTests` 8/8 on real PG (R9 000 operator example, outage+retry-once, partial, full reversal, over-allocation refusal, link refusal, close-guard, randomised 200-op net-zero); arch 76/77. Domain touched-areas coverage 82.2% (unit) with infra legs proven by integration. Migrations reversible, `Down` executed on scratch PG for both 07c migrations; `has-pending-model-changes` clean for both contexts. New ADRs: ADR-151 (clearing→allocation id link — the randomised run caught an ambiguous company-plus-amount match leaving a `Pending` clearing dangling), ADR-152 (outbound group payments deferred; port + domain types stay as the contract).

Reasons recorded, not silence: (1) DemoSeed stays single-company — its own documented single-database constraint forbids the 3-company group; the harness seed (3 companies, one Operator ID, posting rules) is the exercised seed. (2) No `[Replicated]` on group receipt entities — uniform with every other registry saga (06c/09b/10c); `DATA_MODEL.md` row stands as intent for registry→cloud sync. (3) ReverseAsync does not re-check links — matches the 09b return precedent (checked at commitment, unwound without re-check). (4) Arch wall-clock failure (`Domain/Crm`, `Domain/Loyalty` scaffolding, commit `b6b83be`) is pre-existing on main and out of scope. (5) money-and-tax + multi-company-guard + architecture-guard done as inline self-review — subagent panel unlaunchable in this environment; `stage-verifier` + exit checklist still to run before Stage 07c is DONE. Follow-up for a live tenant: production posting rules for the `group.receipt.*` / `inter-company.clearing.*` event types need a home.
### Storefront checkout expiry boundary — 2026-09-13

Closed a correctness gap in Stage 21 checkout orchestration: confirmation and rejection now enforce
the 24-hour intent expiry inside the aggregate, making the boundary safe when the expiry hosted
...
- Incremental solution build after the analyzer cleanup: 0 warnings and 0 errors. A forced clean
  ...
  open because suppressing them would violate the remediation rule against blanket warning
  suppression. Domain/Application still build with zero warnings/errors.
